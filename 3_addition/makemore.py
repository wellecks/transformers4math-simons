...
def print_samples(num=10, new_only_print=False):
    ...
    for b in range(num_batches):
        X_init = torch.zeros(batch_size, 1, dtype=torch.long).to(args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-samples', type=int, default=50, help="number of samples when --sample-only is set")
    parser.add_argument('--num-workers', '-n', type=int, default=4, help="number of data workers for both train/test")
    parser.add_argument('--max-steps', type=int, default=50000, help="max number of optimization steps to run for, or -1 for infinite.")
    ...
    if args.sample_only:
        print_samples(num=args.num_samples, new_only_print=False)
        sys.exit()
